...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=f"""
Examples:
  {cli_name} run                                                          # Start scheduler with frontend
  {cli_name} run -m {{model-name}} -n {{number-of-worker-nodes}}              # Start scheduler without frontend
  {cli_name} run -m Qwen/Qwen3-0.6B -n 2                                  # example
  {cli_name} join                                                         # Join cluster in local network
  {cli_name} join -s {{scheduler-address}}                                  # Join cluster in public network
  {cli_name} join -s 12D3KooWLX7MWuzi1Txa5LyZS4eTQ2tPaJijheH8faHggB9SxnBu # example
...
Note: Both 'toolkit-mesh' and 'parallax' commands are available (toolkit-mesh is the primary Toolkit-branded CLI)
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"Toolkit Inference Mesh v{get_current_version()} (fork of Parallax by Gradient Network)",
    )
    ...
    join_parser = subparsers.add_parser(
        "join", help="Join an Toolkit Inference Mesh cluster as a worker node"
    )
