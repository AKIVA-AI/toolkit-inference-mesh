"""
Control-plane adapter for toolkit-inference-mesh.
...
Toolkit-inference-mesh is an Archetype 9 CLI tool (TK-IM).  The control-plane
contract here covers:
  - Config hierarchy (global defaults -> toolkit config -> CLI overrides)
  - CLI command -> ToolSpec mapping for run, join, chat
