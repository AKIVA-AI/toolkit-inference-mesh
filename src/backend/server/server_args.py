...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Optional JSONL file path to append Toolkit inference event logs (schema v1).",
    )
    parser.add_argument(
        "--toolkit-cost-per-1k-tokens-usd",
        type=float,
        default=0.0,
        help="Optional cost estimator used to populate cost_usd in Toolkit inference events.",
    )

    args = parser.parse_args()

    # Normalize paths early for easier downstream usage.
