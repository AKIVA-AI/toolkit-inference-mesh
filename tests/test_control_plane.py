"""
Tests for toolkit-inference-mesh control_plane adapter.
...
from parallax.control_plane.config import (
    CONFIG_LEVELS,
    ToolkitConfigContract,
    build_config_hierarchy,
)
from parallax.control_plane.contracts import (
...
from parallax.control_plane.tool_specs import (
    TOOLKIT_TOOL_SPECS,
    get_tool_spec,
)
...
class TestToolSpec:
    def test_construction(self) -> None:
        ...
        assert spec.permission_scope == PermissionScope.FULL_ACCESS
...
class TestBuildConfigHierarchy:
    def test_defaults(self) -> None:
        ...
        assert cfg.toolkit_id == "TK-IM"
        assert cfg.toolkit_name == "toolkit-inference-mesh"
        assert cfg.log_format == "json"
        assert cfg.structured_logging is True
        assert cfg.scheduler_port == 7589
        assert cfg.num_nodes == 1

    def test_toolkit_config_overrides_defaults(self) -> None:
        cfg = build_config_hierarchy(toolkit_config={"scheduler_port": 8000, "num_nodes": 4})
        assert cfg.scheduler_port == 8000
        assert cfg.num_nodes == 4
        assert cfg.toolkit_id == "TK-IM"

    def test_cli_overrides_toolkit_config(self) -> None:
        cfg = build_config_hierarchy(
            toolkit_config={"num_nodes": 4},
            cli_overrides={"num_nodes": 8},
        )
...
class TestToolkitToolSpecs:
    ...
    def test_run_requires_model(self) -> None:
        schema = TOOLKIT_TOOL_SPECS["run"].spec.input_schema
        assert schema is not None
        assert "model" in schema.get("required", [])

    def test_join_requires_model(self) -> None:
        schema = TOOLKIT_TOOL_SPECS["join"].spec.input_schema
        assert schema is not None
        assert "model" in schema.get("required", [])

    def test_chat_requires_model(self) -> None:
        schema = TOOLKIT_TOOL_SPECS["chat"].spec.input_schema
        assert schema is not None
        assert "model" in schema.get("required", [])
    ...
    def test_owner_is_toolkit(self) -> None:
        for cmd_spec in TOOLKIT_TOOL_SPECS.values():
            assert cmd_spec.spec.owner == "toolkit-inference-mesh"
